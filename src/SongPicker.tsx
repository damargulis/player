...
export default class SongPicker extends React.Component<SongPickerProps,SongPickerState> {
  ...
  componentDidUpdate(prevProps: SongPickerProps) {
    if (prevProps.songs !== this.props.songs) {
      this.sort(this.state);
    }
  }

  sortSongs(sortBy: string, sortDirection: string) {
  ...
  getRowStyle({index}: {index: number}): React.CSSProperties {
    const style = {
  ...
  doRowRightClick({index}: {index: number}) {
    if (!this.state.selected.includes(index)) {
  ...
  onRowClick({event, index}: {event: {ctrlKey: boolean, shiftKey: boolean, metaKey: boolean}, index: number}) {
    if (event.shiftKey) {
  ...
  onRowDoubleClick({event, index}: {event: {shiftKey: boolean, metaKey: boolean, ctrlKey: boolean}, index: number}) {
    if (!event.shiftKey && !event.ctrlKey && !event.metaKey) {
      this.doDoubleClickSong(index);
    }
  }

  doClickSong(index: number) {
    this.setState({
      selected: [index],
      lastSelected: index,
    });
  ...
  doDoubleClickSong(index: number) {
    const song = this.state.songs[index];
  ...
  sort({sortBy, sortDirection}: {sortBy: string, sortDirection: Sort}) {
    const songs = this.sortSongs(sortBy, sortDirection);
